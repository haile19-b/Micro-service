...
export class OrderService {
  ...
  static async listenForFeedback() {
    ...
    await RabbitMQ.consumeMessage("order.stock.reserved", async (payload: { orderId: string; totalPrice: number }) => {
    ...
    await RabbitMQ.consumeMessage("order.stock.failed", async (payload: { orderId: string; reason: string }) => {
